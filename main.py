...
import random
facts_list = ["В настоящее время в России находится больше стодолларовых купюр, чем в США.", 'Есть только 13 дирижаблей в мире. Девять из них находятся в Соединенных Штатах.', 'В Дании проходят испытания Walking House – самоходного дома, разработанного общественной организацией N55. Жилищная система Walking House (Гуляющий дом) способна передвигаться со скоростью до 60 метров в час.', 'Самый большой рыбный косяк образовали сельди. В нём насчитывалось около 3.000.000.000 особей рыб. ', 'Жирафы спят от 10 минут до 2 часов в день.', 'Пчелы никогда не спят. Пчела замирает на соте лишь на несколько секунд в течение суток.', 'В английских игорных домах были специальные люди, которые должны были глотать игральные кости при появлении полиции.', 'Самый везучий человек на Земле Хорват Фране Селак пережил 7 катастроф, а затем выйграл в лотерию около $1.000.000 и женился на женщине, моложе его на 20 лет.']
app = Flask(__name__)
@app.route("/")
def hello():
    return '''<h1>Привет! Ты попал на сайт с разными темами. Например: 1. Случайные факты обо всём, 2. Что такое Python.</h1>
    <h3><a href="/facts" class="button">Посмотреть слуайный факт!</a></h3>'''
@app.route("/facts")
def hello_world():
...
def python():
    ...
    <header>
        ...
        <div class="zagalovok">
            <img src="https://avatars.mds.yandex.net/i?id=71c05fefea0ea22aedd7858e62ee197cca6f6a20-10491961-images-thumbs&n=13" alt="Логотип Python" width="10%">
            <h1 class ="text">Python</h1> 
            <iframe class = "weather" src="https://www.meteoblue.com/en/weather/widget/daily?geoloc=detect&days=2&tempunit=CELSIUS&windunit=KILOMETER_PER_HOUR&precipunit=MILLIMETER&coloured=coloured&pictoicon=0&pictoicon=1&maxtemperature=0&maxtemperature=1&mintemperature=0&mintemperature=1&windspeed=0&windspeed=1&windgust=0&winddirection=0&winddirection=1&uv=0&humidity=0&precipitation=0&precipitation=1&precipitationprobability=0&precipitationprobability=1&spot=0&spot=1&pressure=0&layout=light"  frameborder="0" scrolling="NO" allowtransparency="true" sandbox="allow-same-origin allow-scripts allow-popups allow-popups-to-escape-sandbox" style="width: 108px; height: 420px"></iframe><div><!-- DO NOT REMOVE THIS LINK --><a href="https://www.meteoblue.com/en/weather/week/index?utm_source=weather_widget&utm_medium=linkus&utm_content=daily&utm_campaign=Weather%2BWidget" target="_blank" rel="noopener"></a></div>
    </header>
    ...
    <main>
        ...
        <p>Python (МФА: [ˈpʌɪθ(ə)n]; в русском языке встречаются названия пито́н[17] или па́йтон[18]) — высокоуровневый язык программирования общего назначения с динамической строгой типизацией и автоматическим управлением памятью[19][20], ориентированный на повышение производительности разработчика, читаемости кода и его качества, а также на обеспечение переносимости написанных на нём программ[21]. Язык является полностью объектно-ориентированным в том плане, что всё является объектами[19]. Необычной особенностью языка является выделение блоков кода отступами[22]. Синтаксис ядра языка минималистичен, за счёт чего на практике редко возникает необходимость обращаться к документации[21]. Сам же язык известен как интерпретируемый и используется в том числе для написания скриптов[19]. Недостатками языка являются зачастую более низкая скорость работы и более высокое потребление памяти написанных на нём программ по сравнению с аналогичным кодом, написанным на компилируемых языках, таких как C или C++[19][21].
            ...
            Python стал одним из самых популярных языков, он используется в анализе данных, машинном обучении, DevOps и веб-разработке, а также в других сферах, включая разработку игр. За счёт читабельности, простого синтаксиса и отсутствия необходимости в компиляции язык хорошо подходит для обучения программированию, позволяя концентрироваться на изучении алгоритмов, концептов и парадигм. Отладка же и экспериментирование в значительной степени облегчаются тем фактом, что язык является интерпретируемым[19][42]. Применяется язык многими крупными компаниями, такими как Google или Facebook[19].</p>
        <img src="https://i.ytimg.com/vi/ot9We1Ooqio/maxresdefault.jpg" alt="Логотип Python"  width="15%" >
        ...
        <ul>
            ...
            <li>Перегрузка операторов (всех, кроме is, '.', '=' и символьных логических).</li>
            ...
            <li>Методы для управления наиболее распространёнными операциями (истинностное значение, len(), глубокое копирование, сериализация, итерация по объекту, …)</li>
            <li>Полная интроспекция.</li>
            <li>Классовые и статические методы, классовые поля.</li>
            <li>Классы, вложенные в функции и классы.</li>
            <li>Возможность модифицировать объекты во время исполнения программ</li>
        </ul>
    </main>
